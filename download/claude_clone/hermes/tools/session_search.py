"""
Hermes Session Search — full-text search across conversation history.
...
_DB_PATH = Path.home() / ".claude_clone" / "hermes_sessions.db"
_FTSPATH = Path.home() / ".claude_clone" / "hermes_sessions_fts.db"
...
async def hermes_session_store(
...
async def hermes_session_list(limit: int = 20) -> str:
...
async def hermes_session_export(session_id: str, output_path: str = "") -> str:
...
ToolRegistry.instance().register(
    name="hermes_session_store",
    func=hermes_session_store,
...
ToolRegistry.instance().register(
    ...
    func=hermes_session_search,
    description="Full-text search across all indexed conversation sessions.",
    toolset="session",
)

ToolRegistry.instance().register(
    name="hermes_session_list",
    func=hermes_session_list,
    description="List all indexed conversation sessions.",
    toolset="session",
)

ToolRegistry.instance().register(
    name="hermes_session_export",
    func=hermes_session_export,
    description="Export a session's messages to a JSON file.",
    toolset="session",
)

ToolRegistry.instance().register(
    name="hermes_session_delete",
    func=hermes_session_delete,
