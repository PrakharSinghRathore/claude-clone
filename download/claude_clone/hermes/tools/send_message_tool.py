"""
Hermes Send Message Tool — cross-platform messaging.
...
_MSG_CONFIG_PATH = Path.home() / ".claude_clone" / "hermes_messaging.json"
...
async def hermes_configure_platform(
    platform: str,
    credentials: str,
) -> str:
...
async def hermes_message_history(platform: str = "", limit: int = 20) -> str:
...
ToolRegistry.instance().register(
    ...
    func=hermes_send_message,
    description="Send a message to Telegram, Discord, Slack, or a generic webhook.",
    toolset="messaging",
)

ToolRegistry.instance().register(
    name="hermes_configure_platform",
    func=hermes_configure_platform,
    description="Store credentials for a messaging platform.",
    toolset="messaging",
)

ToolRegistry.instance().register(
    name="hermes_message_history",
    func=hermes_message_history,
