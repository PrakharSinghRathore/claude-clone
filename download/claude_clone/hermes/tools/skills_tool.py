"""
Hermes Skills Tool — skill management interface.
...
async def hermes_skill_list(category: str = "") -> str:
...
async def hermes_skill_create(
    ...
    def _do():
        ...
        if skill_dir.exists():
            return f"Error: Skill '{name}' already exists. Use hermes_skill_update to modify it."
...
async def hermes_skill_execute(name: str, input_data: str = "") -> str:
    ...
    def _do():
        ...
        if not manifest_path.exists():
            return f"Error: Skill '{name}' not found. Use hermes_skill_list to see available skills."
...
async def hermes_skill_uninstall(name: str) -> str:
    """Uninstall a skill.
...
async def hermes_skill_update(name: str, instructions: str = "", description: str = "") -> str:
...
async def hermes_skill_export(name: str, output_path: str = "") -> str:
...
async def hermes_skill_import(path: str) -> str:
...
ToolRegistry.instance().register(
    ...
    func=hermes_skill_update,
    description="Update an existing skill's instructions or description.",
    toolset="skills",
)

ToolRegistry.instance().register(
    name="hermes_skill_export",
    func=hermes_skill_export,
    description="Export a skill as a JSON file for sharing.",
    toolset="skills",
)

ToolRegistry.instance().register(
    name="hermes_skill_import",
    func=hermes_skill_import,
