"""
Atlas Skills Hub — browse and install skills from a community marketplace.
...
async def atlas_hub_browse(category: str = "", sort_by: str = "downloads") -> str:
...
async def atlas_hub_search(query: str) -> str:
...
async def atlas_hub_install(name: str) -> str:
    ...
    from atlas.tools.skills_tool import atlas_skill_create

    result = await atlas_skill_create(
        name=skill["name"],
    ...
    return f"{result}\nInstalled from Atlas Skills Hub (downloads: {skill['downloads']})"


async def atlas_hub_info(name: str) -> str:
...
async def atlas_hub_rate(name: str, rating: int = 5, review: str = "") -> str:
