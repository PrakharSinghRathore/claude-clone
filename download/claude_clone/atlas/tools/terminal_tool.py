"""
Atlas Terminal Tool — shell command execution with multiple backends.
...
async def atlas_run_command_ssh(
    command: str,
    host: str,
    username: str = "",
    port: int = 22,
...
async def atlas_command_history(limit: int = 20) -> str:
...
async def atlas_replay_command(index: int) -> str:
    ...
    return await atlas_run_command(
...
ToolRegistry.instance().register(
    name="atlas_run_command",
    func=atlas_run_command,
    description="Execute a shell command locally with timeout and output capture.",
    toolset="terminal",
)

ToolRegistry.instance().register(
    name="atlas_run_command_ssh",
    func=atlas_run_command_ssh,
    description="Execute a shell command on a remote host via SSH.",
    toolset="terminal",
)

ToolRegistry.instance().register(
    name="atlas_command_history",
    func=atlas_command_history,
    description="Show recent command execution history.",
    toolset="terminal",
)

ToolRegistry.instance().register(
    name="atlas_replay_command",
    func=atlas_replay_command,
