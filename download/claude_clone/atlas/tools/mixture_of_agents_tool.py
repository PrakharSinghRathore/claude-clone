"""
Atlas Mixture of Agents Tool — multi-model consensus for better outputs.
...
async def atlas_mixture_of_agents(
    prompt: str,
    models: str = "",
...
async def _query_model(
    ...
    headers = {
        ...
        "HTTP-Referer": "https://github.com/claude-clone/atlas",
    }
...
async def atlas_model_list() -> str:
    ...
    lines.append("\nUsage: Pass comma-separated model names to atlas_mixture_of_agents")
    return "\n".join(lines)
...
ToolRegistry.instance().register(
    name="atlas_mixture_of_agents",
    func=atlas_mixture_of_agents,
    description="Send a prompt to multiple AI models and aggregate responses with consensus.",
    toolset="agent",
)

ToolRegistry.instance().register(
    name="atlas_model_list",
    func=atlas_model_list,
