"""
Atlas Delegate Tool — spawn subagents for parallel work.
...
# ---------------------------------------------------------------------------
# Simple subagent execution (uses the atlas tool system internally)
# ---------------------------------------------------------------------------
...
async def atlas_delegate_status(agent_id: str = "") -> str:
...
async def atlas_delegate_results(agent_ids: str = "") -> str:
...
async def atlas_delegate_cancel(agent_id: str) -> str:
...
ToolRegistry.instance().register(
    name="atlas_delegate_status",
    func=atlas_delegate_status,
    description="Check status of a subagent or list all active agents.",
    toolset="agent",
)

ToolRegistry.instance().register(
    name="atlas_delegate_results",
    func=atlas_delegate_results,
    description="Aggregate results from completed subagents.",
    toolset="agent",
)

ToolRegistry.instance().register(
    name="atlas_delegate_cancel",
    func=atlas_delegate_cancel,
