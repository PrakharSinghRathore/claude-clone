"""
Atlas Cron Job Tool — scheduled task management.
...
_CRON_DB_PATH = Path.home() / ".claude_clone" / "atlas_cron.json"
...
async def atlas_cron_remove(name: str) -> str:
    """Remove a cron job.
...
async def atlas_cron_pause(name: str) -> str:
    """Pause a cron job.
...
async def atlas_cron_resume(name: str) -> str:
...
async def atlas_cron_logs(name: str = "", limit: int = 20) -> str:
...
ToolRegistry.instance().register(
    name="atlas_cron_create",
    func=atlas_cron_create,
    description="Create a new cron job with natural language or cron expression scheduling.",
    toolset="automation",
)

ToolRegistry.instance().register(
    name="atlas_cron_list",
    func=atlas_cron_list,
    description="List all cron jobs with their status and run history.",
    toolset="automation",
)

ToolRegistry.instance().register(
    name="atlas_cron_remove",
    func=atlas_cron_remove,
    description="Remove a cron job by name.",
    toolset="automation",
)

ToolRegistry.instance().register(
    name="atlas_cron_pause",
    func=atlas_cron_pause,
    description="Pause a running cron job.",
    toolset="automation",
)

ToolRegistry.instance().register(
    name="atlas_cron_resume",
    func=atlas_cron_resume,
    description="Resume a paused cron job.",
    toolset="automation",
)

ToolRegistry.instance().register(
    name="atlas_cron_logs",
    func=atlas_cron_logs,
