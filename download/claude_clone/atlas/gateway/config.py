...
Provides Pydantic-based configuration classes for the Atlas gateway,
...
class GatewayConfig:
    """
    Top-level configuration for the Atlas Gateway.
    ...
    def from_env(cls) -> "GatewayConfig":
        ...
        Environment variable naming convention:
            ``ATLAS_<SECTION>_<KEY>`` (e.g., ``ATLAS_SESSION_TIMEOUT``)

        Platform-specific variables:
            ``ATLAS_TELEGRAM_TOKEN``, ``ATLAS_DISCORD_TOKEN``, etc.
        """
        ...
        for name in platform_names:
            env_prefix = f"ATLAS_{name.upper()}"
        ...
        return cls(
            platforms=platforms,
            session_timeout=int(os.environ.get("ATLAS_SESSION_TIMEOUT", "3600")),
            session_max_tokens=int(os.environ.get("ATLAS_SESSION_MAX_TOKENS", "100000")),
            session_persist_path=os.environ.get("ATLAS_SESSION_PERSIST_PATH"),
            session_reset_policy=os.environ.get("ATLAS_SESSION_RESET_POLICY", "token_limit"),
            delivery_retry_count=int(os.environ.get("ATLAS_DELIVERY_RETRY_COUNT", "3")),
            delivery_retry_delay=float(os.environ.get("ATLAS_DELIVERY_RETRY_DELAY", "2.0")),
            streaming_enabled=os.environ.get("ATLAS_STREAMING_ENABLED", "true").lower() in ("true", "1", "yes"),
            streaming_chunk_size=int(os.environ.get("ATLAS_STREAMING_CHUNK_SIZE", "100")),
            mirroring_enabled=os.environ.get("ATLAS_MIRRORING_ENABLED", "false").lower() in ("true", "1", "yes"),
            hooks_enabled=os.environ.get("ATLAS_HOOKS_ENABLED", "true").lower() in ("true", "1", "yes"),
            pairing_enabled=os.environ.get("ATLAS_PAIRING_ENABLED", "true").lower() in ("true", "1", "yes"),
            pairing_secret=os.environ.get("ATLAS_PAIRING_SECRET"),
            admin_secret=os.environ.get("ATLAS_ADMIN_SECRET"),
            log_level=os.environ.get("ATLAS_LOG_LEVEL", "INFO"),
            health_check_interval=int(os.environ.get("ATLAS_HEALTH_CHECK_INTERVAL", "60")),
            status_port=int(os.environ.get("ATLAS_STATUS_PORT", "9090")),
            worker_threads=int(os.environ.get("ATLAS_WORKER_THREADS", "4")),
            gateway_host=os.environ.get("ATLAS_GATEWAY_HOST", "0.0.0.0"),
            gateway_port=int(os.environ.get("ATLAS_GATEWAY_PORT", "8000")),
        )
    ...
    def _from_parsed(cls, data: Dict[str, Any]) -> "GatewayConfig":
        ...
        return cls(
            ...
            pairing_secret=os.environ.get("ATLAS_PAIRING_SECRET") or data.get("pairing_secret"),
            admin_secret=os.environ.get("ATLAS_ADMIN_SECRET") or data.get("admin_secret"),
