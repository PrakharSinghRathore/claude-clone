"""
CLI event callbacks for Atlas TUI.
...
class CallbackManager:
    """Manages event callbacks for the Atlas CLI."""
...
class StandardCallbacks:
    ...
    def on_completion(self, event: CallbackEvent) -> None:
        ...
        if self.manager.notification_enabled:
            self._show_notification("Atlas CLI", "Response completed")
    ...
    def on_notification(self, event: CallbackEvent) -> None:
        ...
        if self.manager.notification_enabled:
            data = event.data or {}
            title = data.get("title", "Atlas CLI")
