"""
Diagnostic tool (doctor) for Atlas CLI.
...
from atlas.cli_atlas.config_manager import ConfigManager, DEFAULT_CONFIG_DIR
...
class Doctor:
    """System diagnostic tool for Atlas CLI."""
    ...
    def fix_issues(self) -> List[str]:
        ...
        # Ensure atlas subdirectories exist
        atlas_dir = DEFAULT_CONFIG_DIR / "atlas"
        for subdir in ["profiles", "themes", "sessions", "cron", "skills"]:
            (atlas_dir / subdir).mkdir(parents=True, exist_ok=True)
        fixes.append("Created atlas subdirectories")
    ...
    def check_config_file(self):
        ...
        atlas_config = DEFAULT_CONFIG_DIR / "atlas" / "config.yaml"

        if config_file.exists() or atlas_config.exists():
            try:
                errors, warnings = self.config.validate()
                if errors:
                    self._results.append(DiagnosticResult(
    ...
    def check_theme(self):
        ...
        if theme in ("dark", "light"):
            ...
            theme_file = Path.home() / ".claude_clone" / "atlas" / "themes" / f"{skin}.yaml"
